class Activity:
    def __init__(self, name, tag, energy, duration, time_requirement, set_time=None):
        self.name = name
        self.tag = tag
        self.energy = energy
...
class TimeBlock:
    def __init__(self, time_start, duration, energy, is_free=False, activity=None):
    ...
    def __repr__(self):
        return f"{self.__time_start}~{self.__time_start + self.__duration}"
...
class DayBlock:
    def __init__(self, name, is_weekend=False):
        ...
        if self.__is_weekend:
            self.times = [TimeBlock(Time(h,m), Time(0,30), "medium") for h in range(7,24) for m in (0,30)]
        else:
            self.times = [TimeBlock(Time(h,m), Time(0,30), "medium") for h in range(5,24) for m in (0,30)]
